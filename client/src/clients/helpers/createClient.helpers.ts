...
export type CreateClientFormType = {
  type: string;
  name: string;
  phone: string;
  email: string;
...
export const createClientInitialData: CreateClientFormType = {
  type: "private",
  name: "",
  phone: "",
  email: "",
...
export const createClientSchema = Joi.object<CreateClientFormType>({
  ...
  billingPhone: Joi.string().pattern(phonePattern).allow("").optional().messages({
