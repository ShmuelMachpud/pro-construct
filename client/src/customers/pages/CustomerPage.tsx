...
const CustomerPage = () => {
  ...
  const { customer, projects, loading, error } = useCustomerPage(id!);

  if (loading) return <Box sx={{ display: "flex", justifyContent: "center", mt: 10 }}><CircularProgress /></Box>;
  if (!customer) return <Typography color="error" sx={{ mt: 4 }}>{error || "לקוח לא נמצא"}</Typography>;
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 4 }}>
        ...
        <Typography variant="h5" fontWeight="bold" color="white">{customer.name}</Typography>
        <Chip
          ...
          <Grid size={6}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              <Box>
                ...
                <Typography color="white" fontSize="0.9rem">{customer.phone}</Typography>
              </Box>
            </Box>
          </Grid>
          <Grid size={6}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              <Box>
                ...
                <Typography color="white" fontSize="0.9rem">{customer.email}</Typography>
              </Box>
            </Box>
          </Grid>
          <Grid size={12}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              <Box>
                ...
                <Typography color="white" fontSize="0.9rem">{customer.address}</Typography>
              </Box>
            </Box>
          </Grid>
        </Grid>

        {(customer.billingName || customer.billingPhone || customer.billingAddress) && (
          <>
            ...
            <Grid container spacing={2}>
              {customer.billingName && (
                <Grid size={6}>
                  ...
                  <Typography color="white" fontSize="0.9rem">{customer.billingName}</Typography>
                </Grid>
              )}
              {customer.billingPhone && (
                <Grid size={6}>
                  <Typography color="grey.600" fontSize="0.72rem">טלפון לחיוב</Typography>
                  <Typography color="white" fontSize="0.9rem">{customer.billingPhone}</Typography>
                </Grid>
              )}
              {customer.billingAddress && (
                <Grid size={12}>
                  <Typography color="grey.600" fontSize="0.72rem">כתובת לחיוב</Typography>
                  <Typography color="white" fontSize="0.9rem">{customer.billingAddress}</Typography>
                </Grid>
              )}
            </Grid>
          </>
